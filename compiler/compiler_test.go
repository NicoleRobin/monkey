...
func TestCompilerScope(t *testing.T) {
	...
	if previous.Opcode != code.OpMul {
		t.Errorf("previous.Opcode wrong. got=%d, want=%d",
...
func TestLetStatementScopes(t *testing.T) {
	tests := []compilerTestCase{
		...
		{
			// 断言创建、访问局部变量会产生OpSetLocal OpGetLocal
			input: `
			fn() {
				let num = 55;
				num
...
func testConstants(expected []interface{}, actual []object.Object) error {
	...
	for i, constant := range expected {
		...
		case string:
			...
			if err != nil {
				return fmt.Errorf("constant %d - testStringObject failed, error:%s",
					i, err)
			}
		case []code.Instructions:
			...
			if err != nil {
				return fmt.Errorf("constant %d - testInstructions() failed, error:%s", i, err)
			}
		}
	}
	return nil
...
func runCompilerTests(t *testing.T, tests []compilerTestCase) {
	...
	for _, tt := range tests {
		...
		if err != nil {
			t.Fatalf("testInstructions() failed, input:%s, error: %s", tt.input, err)
		}

		err = testConstants(tt.expectedConstants, bytecode.Constants)
		if err != nil {
			t.Fatalf("testConstants() failed, input:%s, error: %s", tt.input, err)
		}
	}
	// bpf.Instruction
}
